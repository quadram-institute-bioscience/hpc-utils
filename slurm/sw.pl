...
print STDERR color('green'), "
 ------------------------------------------------------------------------------------
  SlurmWrapper: Prepare job for SLURM scheduler at NBI/QIB
 ------------------------------------------------------------------------------------",
color('reset');
...
say STDERR<<END;
 ...
 General use:
 sw.pl [options]  'your unix command'
...
sub init {
	# Check that jobs/logs directories are accessible: put here all directories the programs rely on
...
sub check_dir {
	# Check dir exists -> in case try creating one -> Die if it fails
...
sub read_content_from_file {
	# Like slurp, but without needing yet another module 
	my $file = $_[0];
	if (-e "$file") {
